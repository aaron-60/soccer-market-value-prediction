"""
Inject dashboard visuals into PBI Desktop's generated PBIP report structure.
Uses PBI's own infrastructure files (report.json, version.json, pages.json, .platform)
and adds visual.json files in the correct format for schema v2.0+.
"""
import json, os, uuid, secrets

REPORT_DIR = os.path.join(
    os.path.dirname(os.path.dirname(__file__)),
    "pbip_project", "SoccerDashboard.Report", "definition"
)

def uid():
    return secrets.token_hex(10)  # PBI uses hex IDs, not UUIDs

def write_json(path, obj):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w", encoding="utf-8") as f:
        json.dump(obj, f, indent=2, ensure_ascii=False)
    print(f"  Created: {os.path.relpath(path, REPORT_DIR)}")
...
# ---------------------------------------------------------------------------
# Visual builders (same logic, compatible with PBI v2.0 format)
# ---------------------------------------------------------------------------
...
def make_card_visual(name, x, y, w, h, table, measure_name, label_color=ACCENT):
    ...
    return {
        "$schema": "https://developer.microsoft.com/json-schemas/fabric/item/report/definition/visual/2.5.0/schema.json",
        "name": name,
...
# ---------------------------------------------------------------------------
# Main: inject visuals into PBI's generated structure
# ---------------------------------------------------------------------------

def main():
    print("Injecting visuals into PBI Desktop's report structure...")

    # Read PBI's pages.json to find the page ID
    pages_json_path = os.path.join(REPORT_DIR, "pages", "pages.json")
    with open(pages_json_path, "r", encoding="utf-8") as f:
        pages_meta = json.load(f)

    page_id = pages_meta["pageOrder"][0]
    print(f"  Using PBI-generated page: {page_id}")

    page_dir = os.path.join(REPORT_DIR, "pages", page_id)

    # Update page.json with dark theme and better name
    ...
    visuals.append(make_textbox(uid(), 15, 5, 550, 48, "Football Player Market Value Analysis"))
    visuals.append(make_card_visual(uid(), 700, 5, 135, 55, "Players", "Total Players", ACCENT))
    visuals.append(make_card_visual(uid(), 845, 5, 175, 55, "Players", "Total Market Value", ACCENT2))
    visuals.append(make_card_visual(uid(), 1030, 5, 120, 55, "Players", "Top Player Value", ACCENT3))
    visuals.append(make_card_visual(uid(), 1160, 5, 110, 55, "Predictions", "Prediction Accuracy", "#CE93D8"))

    # Row 2: Slicers (y=65, h=30)
    visuals.append(make_slicer(uid(), 15, 65, 200, 32, "Players", "position"))
    visuals.append(make_slicer(uid(), 225, 65, 200, 32, "Players", "league_tier"))
    visuals.append(make_slicer(uid(), 435, 65, 200, 32, "Players", "age_group"))

    # Row 3: Map + Bar + Donut (y=102, h=295)
    visuals.append(make_map_visual(uid(), 15, 102, 590, 295))
    visuals.append(make_bar_chart(uid(), 615, 102, 400, 295))
    visuals.append(make_donut(uid(), 1025, 102, 245, 295))

    # Row 4: Table + Scatter (y=405, h=305)
    visuals.append(make_table_visual(uid(), 15, 405, 645, 305))
    visuals.append(make_scatter(uid(), 670, 405, 600, 305))

    # Write visual files into PBI's page folder
    visuals_dir = os.path.join(page_dir, "visuals")
    for v in visuals:
        vdir = os.path.join(visuals_dir, v["name"])
        write_json(os.path.join(vdir, "visual.json"), v)

    print(f"\nDone! Injected {len(visuals)} visuals into page '{page_id}'.")
    print("Reopen PBI Desktop with the .pbip file to see the dashboard.")
